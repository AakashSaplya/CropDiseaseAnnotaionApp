...
class AuthController extends GetxController {
  ...
  _initialScreen(User? user) async {
    ...
    if (user == null) {
      ...
      if (user.email != null) {
        ...
        print('phoneNumber print ${user.phoneNumber}');
        // Check user already  in the Firestore database
        DocumentSnapshot userSnapshot = await FirebaseFirestore.instance
            .collection('users')
        ...
        if (userSnapshot.exists) {
          ...
          if (userData != null) {
            ...
            //according to category open UI
            if (category == 'farmer') {
              //Get.offAll(() => AskLocation(email: user.phoneNumber!));
              Get.offAll(() => HomePage(email: user.phoneNumber!));
            } else if (category == 'admin') {
              Get.offAll(() => DashboardAdm(email: user.phoneNumber!));
            } else if (category == 'annotator') {
              Get.offAll(() => HomePageAnn(
  ...
  void signinWithPhoneNumber(verificationid, smscode) async {
    ...
    try {
      ...
      if (user != null) {
        // Access user properties
        String email = user.phoneNumber.toString();
        //check user exist in firestore or not
        DocumentSnapshot userSnapshot = await FirebaseFirestore.instance
            .collection('users')
            .doc(email)
